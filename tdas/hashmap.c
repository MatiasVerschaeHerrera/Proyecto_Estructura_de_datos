...
int has_is_equal(void* key1, void* key2){
...
HashMap * createMap(long capacity) {
    ...
    Mapa -> buckets = (HashPair**)calloc(capacity,sizeof(HashPair*));
...
void insertMap(HashMap * map, char * key, void * value) {
    ...
    if(map ->buckets[posicion] == NULL || map -> buckets[posicion] -> key == NULL){
        HashPair * Nuevo = (HashPair *)malloc(sizeof(HashPair));
        Nuevo -> value = value;
        Nuevo -> key = key;
...
HashPair * searchMap(HashMap * map,  char * key) {   
    ...
    else if(map->buckets[posicion]->key != NULL && strcmp(map -> buckets[posicion] -> key, key) == 0){
...
HashPair * nextMap(HashMap * map) {
    ...
    for(int posicion = map->current + 1 ; posicion < map -> capacity; posicion++){
        if(map -> buckets[posicion] != NULL && map -> buckets[posicion]->key != NULL){
            map -> current = posicion;
            return map -> buckets[posicion];
        }
